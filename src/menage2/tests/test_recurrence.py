...
def test_spawn_protocol_after_no_op_for_every(dbsession):
    ...
    assert spawn_protocol_after(run, datetime.date(2026, 5, 1), _now(), dbsession) is None
...
def test_spawn_protocol_every_on_completion_creates_next(dbsession):
    ...
    futures = (
        ...
        .filter(ProtocolRun.protocol_id == p.id, Todo.due_date >= today + datetime.timedelta(days=1))
        .all()
    )
...
def test_daily_sweep_skips_archived_protocols(dbsession):
    ...
    p.archived_at = _now()
    spawn_protocol_run(p, today - datetime.timedelta(days=14), _now(), dbsession)
    before = dbsession.query(ProtocolRun).filter(ProtocolRun.protocol_id == p.id).count()
    spawn_due_every_if_needed(dbsession, today, _now())
...
def test_ensure_protocol_has_run_creates_first_run(dbsession):
    ...
    assert len(runs) >= 1
    todos = [dbsession.query(Todo).filter(Todo.protocol_run_id == r.id).one() for r in runs]
...
def test_ensure_protocol_has_run_no_op_without_recurrence(dbsession):
    ...
    ensure_protocol_has_run(p, today, _now(), dbsession)
    assert dbsession.query(ProtocolRun).filter(ProtocolRun.protocol_id == p.id).count() == 0
