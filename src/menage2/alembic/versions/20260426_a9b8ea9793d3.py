...
def upgrade():
    op.create_table(
        ...
        sa.Column('kind', sa.Enum('after', 'every', name='recurrencekind'), nullable=False),
        sa.Column('interval_value', sa.Integer(), nullable=False),
        sa.Column('interval_unit', sa.Enum('day', 'week', 'month', 'year', name='recurrenceunit'), nullable=False),
...
def downgrade():
    op.drop_constraint(op.f('fk_todos_recurred_from_id_todos'), 'todos', type_='foreignkey')
    op.drop_constraint(op.f('fk_todos_recurrence_id_recurrence_rules'), 'todos', type_='foreignkey')
