    ...
    encodeParameters: function (xhr, parameters, elt) {
      ...
      elt.querySelectorAll('input[type="checkbox"]').forEach((input) => {
        ...
        if (group.length > 1 || wantsList) {
          ...
          if (input.checked) {
            ...
            api.hasAttribute(elt, _ConfigIgnoreDropFalseOptionArray_)
          ) {
      ...
      for (const [key, value] of parameters.entries()) {
        ...
        const transformedValue = input
          ? convertValue(input, value, input.type)
          : value;
      ...
      Object.keys(object).forEach(function (key) {
        object[key] = Object.prototype.hasOwnProperty.call(vals, key)
          ? vals[key]
          : object[key];
      });
